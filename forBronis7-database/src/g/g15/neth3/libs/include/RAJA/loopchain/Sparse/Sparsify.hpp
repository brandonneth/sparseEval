...
auto sparsification_follow_dimensions_compressed( SparseView<ElmType,ViewDims,ViewFormat> & A, std::vector<camp::idx_t> argumentOrderInverse, CompressedLeadSegmentImpl * lead) {
  if constexpr (CurrIdx == MaxIdx) {
    camp::sink(A,argumentOrderInverse,lead);
    return make_tuple();
  } else {
    ...
    auto remainingDims = sparsification_follow_dimensions_compressed<ElmType, ViewDims, ViewFormat, CurrIdx+1, MaxIdx>(A, argumentOrderInverse, lead);
    return tuple_cat(camp::tuple(currDimension), remainingDims);
  }
}
...
auto sparsify_iteration_space_compressed(camp::tuple<DenseTupleTypes...> denseSegs, SparseView<ElmType,ViewDims,ViewFormat> & A, std::vector<camp::idx_t> argumentOrder) {
  ...
  auto followDimensions = sparsification_follow_dimensions_compressed<double,ViewDims,ViewFormat,1,ViewDims>(A, q, leadImpl);
