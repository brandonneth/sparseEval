...
template <typename ElmType, std::size_t ViewDims, std::size_t CurrIdx, std::size_t MaxIdx>
auto sparsification_follow_dimensions( SparseView<ElmType,ViewDims> & A, std::vector<camp::idx_t> argumentOrderInverse, LeadSegmentImpl * lead) {
...
template <std::size_t ViewDims, typename ElmType,typename... DenseTupleTypes>
auto sparsify_iteration_space_equal_dims(camp::tuple<DenseTupleTypes...> denseSegs, SparseView<ElmType,ViewDims> & A, std::vector<camp::idx_t> argumentOrder, std::vector<camp::idx_t> nestingOrder) {
  camp::sink(denseSegs);
  ...
  auto applied = apply_permutation(nestingOrderInverse, q);
  A.impl.set_permutation(applied); 
  auto leadImpl = new LeadSegmentImpl(A.impl.dim(q[0]));

  auto followDimensions = sparsification_follow_dimensions<double,ViewDims,1,ViewDims>(A, q, leadImpl);

  auto leadDimension = new SparseSegment(leadImpl);
...
template <std::size_t ViewDims, typename ElmType,typename... DenseTupleTypes>
auto sparsify_iteration_space_smaller_data(camp::tuple<DenseTupleTypes...> denseSegs, SparseView<ElmType,ViewDims> & A, std::vector<camp::idx_t> argumentOrder, std::vector<camp::idx_t> nestingOrder) {
...
template <typename KernelPolicy, std::size_t ViewDims, typename ElmType,typename... DenseTupleTypes>
auto sparsify_iteration_space_larger_data(camp::tuple<DenseTupleTypes...> denseSegs, SparseView<ElmType,ViewDims> & A, std::vector<camp::idx_t> argumentOrder) {
  camp::sink(A, argumentOrder);
  if constexpr (ViewDims <= sizeof...(DenseTupleTypes)) {
...
template <typename KernelPolicy, std::size_t ViewDims, typename ElmType,typename... DenseTupleTypes>
auto sparsify_iteration_space(camp::tuple<DenseTupleTypes...> denseSegs, SparseView<ElmType,ViewDims> & A, std::vector<camp::idx_t> argumentOrder) {
  auto nestingOrder = policy_to_nesting_order<KernelPolicy>();
  if constexpr (ViewDims == sizeof...(DenseTupleTypes)) {
...
template <typename ElmType, std::size_t ViewDims, std::size_t CurrIdx, std::size_t MaxIdx>
auto sparsification_follow_dimensions_compressed( SparseView<ElmType,ViewDims> & A, std::vector<camp::idx_t> argumentOrderInverse, CompressedLeadSegmentImpl * lead) {
  if constexpr (CurrIdx == MaxIdx) {
    camp::sink(A,argumentOrderInverse,lead);
    return make_tuple();
  } else {
    
    auto currDimension = make_compressed_follow_segment(A.impl.dim(argumentOrderInverse[CurrIdx]), lead);
    auto remainingDims = sparsification_follow_dimensions_compressed<ElmType, ViewDims, CurrIdx+1, MaxIdx>(A, argumentOrderInverse, lead);
    return tuple_cat(camp::tuple(currDimension), remainingDims);
  }
}
...
template <typename KernelPolicy, std::size_t ViewDims, typename ElmType,typename... DenseTupleTypes>
auto sparsify_iteration_space_compressed(camp::tuple<DenseTupleTypes...> denseSegs, SparseView<ElmType,ViewDims> & A, std::vector<camp::idx_t> argumentOrder) {
  auto nestingOrder = policy_to_nesting_order<KernelPolicy>();
  ...
  auto leadDimension = make_compressed_lead_segment(A.impl.dim(q[0]));
  auto leadImpl = (CompressedLeadSegmentImpl*)leadDimension.impl;

  auto followDimensions = sparsification_follow_dimensions_compressed<double,ViewDims,1,ViewDims>(A, q, leadImpl);
