...
struct DIAG : public SparseImpl<ElmType, NumDims> {
  ...
  bool is_diagonal_entry(idx_t entryIdx, idx_t idxVal) {
    if constexpr (Idx == NumDims) { 
      return true; 
    } else {
      return idxVal == dim(Idx)[entryIdx] && is_diagonal_entry<Idx+1>(entryIdx, idxVal);
    }
  ...
  bool is_diagonal(camp::tuple<Idxs...> idxs) {
  ...
  ElmType & operator() (camp::tuple<Idxs...> idxs) {
    ...
    if(is_diagonal<0>(idxs)) {
      hits += 1;
      return diag[camp::get<0>(idxs)];
    } else if(is_expected<0>(idxs)) {
      hits += 1;
